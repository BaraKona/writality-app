...
export const getSingleCollabChapter = async (projectId: string, chapterId: string) => {
  const { data } = await collabApi.get(`/${projectId}/${chapterId}`);
  return data;
};

export const deleteCollabChapter = async (userId: string, projectId: string, chapterId: string) => {
  console.log(userId, projectId, chapterId);
  try {
    const { data } = await collabApi.delete(`${userId}/${projectId}/${chapterId}`);
...
export const updateCollabChapterContent = async (
  ...
  try {
    const { data } = await collabApi.patch(`/${projectId}/${chapterId}`, chapter);
...
export const mergeReplaceMain = async (
  ...
  try {
    const { data } = await collabApi.patch(`/merge/replace/${userId}/${projectId}/${chapterId}`, {
      content,
      history,
      dateUpdated,
    });
